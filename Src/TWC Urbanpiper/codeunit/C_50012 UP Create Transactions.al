codeunit 50012 "UP Create Transactions"
{
    trigger OnRun()
    var
        counter: Integer;
        ...
        if header.FindSet() then begin
            repeat begin
                ...
                if not upOrderstatus.FindLast() then begin
                    ...
                    //  receiptNo := IncStr(posview.GetLastReceiptNo(terminal));//Ashish-O
                    l_receiptno := '';
                    PosFunc.ReadLocalVar(l_receiptno);
                    receiptNo := l_receiptno;//N-YM
                    createTransaction(receiptNo, header.order_details_id, header.order_details_order_state, header.order_details_channel, header.order_details_order_type, header.order_details_channel);
    ...
    local procedure createTransaction(receiptNo: text; orderId: BigInteger; orderStatus: Text; orderChannel: Text; orderType: Text; channel: text)
    var
        trans: Record "LSC POS Transaction";
        nullTime: Time;
        nullDate: Date;
    ...
    begin
        ...
        if up_header.FindLast() then begin
            ...
            trans."App Discount ID" := up_header.app_Discount_ID; //AlleRSN 131223
            trans."App Discount Code" := up_header.app_Discount_Code; //AlleRSN 131223
    ...
    var
        ...
        voided: Record "LSC POS Voided Trans. Line";
